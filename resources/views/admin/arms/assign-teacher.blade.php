        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                @foreach($years as $class)
                    ...
                    @foreach($class->arms as $class_with_subject)
                        <tr>
                            <span class="ghost"> {{$counter = $counter + 1}}</span>
                            <th scope="row">{{$counter}}</th>
                            <th scope="row">{{'Year '.$class->slug}}</th>
                            <th scope="row">{{$class_with_subject->name}}</th>
                            <th scope="row " class="@if(auth()->user()->getName($class_with_subject->pivot->user_id) == 'null') empty-data @endif">{{auth()->user()->getName($class_with_subject->pivot->user_id)}}</th>
                            <td>
                                <form id="{{'form'.$class->id.$class_with_subject->id}}" method="post" action="{{route('assign.class.user.store','red')}}">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="year_id" value="{{$class->id}}">
                                    <input type="hidden" name="arm_id" value="{{$class_with_subject->id}}">
                            ...
                            <td>
                                <button class="btn  btn-circle btn-outline-success" type="submit" onclick='submitFunction({{'form'.$class->id.$class_with_subject->id}})'><i class="fas fa-sync-alt"></i></button>
                            </td>
